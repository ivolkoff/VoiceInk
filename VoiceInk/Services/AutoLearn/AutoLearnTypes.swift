...
enum AutoLearnTextNormalizer {
    /// Comparison-only form of text exposed by editable web controls; the
    /// original field value stays untouched for anchoring.
...
enum AutoLearnSettings {
    ...
    static func recordFailure(_ error: Error) {
        ...
        for extra in [nsError.localizedFailureReason, nsError.localizedRecoverySuggestion] {
            if let extra, !extra.isEmpty, !details.contains(extra) {
