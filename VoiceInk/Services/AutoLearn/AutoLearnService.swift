...
actor AutoLearnService {
    ...
    private var providerAvailabilityObservers: [NSObjectProtocol] = []
    ...
    /// Retries deferred reviews when a provider may have become available:
    /// settings changes and saved API keys post different notifications.
    private func observeProviderAvailability() {
        guard providerAvailabilityObservers.isEmpty else { return }
        providerAvailabilityObservers = [Notification.Name.AppSettingsDidChange, .aiProviderKeyChanged].map {
