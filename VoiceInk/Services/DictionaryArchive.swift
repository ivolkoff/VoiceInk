...
extension DictionaryImportExportService {
    ...
    private static func decodeArchive(from data: Data) throws -> DictionaryArchive {
        ...
        guard let formatValue = root["format"] else {
            return try decodeLegacyBackup(from: data)
        }
    ...
    // Dictionary exports from earlier fork builds and full settings backups have no
    // `format` key; both carry BackupFile's vocabularyWords/wordReplacements fields.
    private static func decodeLegacyBackup(from data: Data) throws -> DictionaryArchive {
        ...
        guard
            ...
            legacy.vocabularyWords != nil || legacy.wordReplacements != nil
        else {
            throw DictionaryArchiveError.invalidFile
        }

        return DictionaryArchive(
            vocabulary: (legacy.vocabularyWords ?? []).map {
                DictionaryVocabularyEntry(term: $0.word, createdAt: nil)
            },
            replacements: (legacy.wordReplacements ?? [:])
                .sorted { $0.key < $1.key }
                .map {
                    DictionaryReplacementEntry(
                        sources: WordReplacementVariants.parse($0.key),
                        replacement: $0.value,
                        createdAt: nil
                    )
                }
