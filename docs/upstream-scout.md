...
- **Взято:** ничего — разведочный проход.
- **Рекомендую первым:** #4 (S, закрывает запись с экрана блокировки) и #2 (низкая связность,
  модели словаря совпадают). #1 — самая ценная, но L и по умолчанию шлёт фрагменты текста в
  облачный LLM; решать после того, как станет ясно, сколько правок после вставки реально делается.
...
- Paste in chunks, emotion annotation — хук Auto Learn (#1) ставить в путь вставки форка, а не
  переносить `CursorPaster` апстрима.
